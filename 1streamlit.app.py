    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r") as f:
                state = json.load(f)
                # Convert string keys back to integers for companies and holdings
                if "companies" in state:
                    state["companies"] = {int(k): v for k, v in state["companies"].items()}
                if "players" in state:
                    for player in state["players"].values():
                        player["holdings"] = {int(k): v for k, v in player["holdings"].items()}
                return state
        except:
            return None
    return None
